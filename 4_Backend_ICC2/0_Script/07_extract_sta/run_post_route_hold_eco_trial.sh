...
eco_name="${HOLD_ECO_NAME:-07_extract_sta_hold_eco_open_site_m0}"
...
if [ -f "$lock_file" ]; then
  echo "ERROR: ICC2 design library is locked. Do not start hold ECO trial."
...
export HOLD_ECO_REPORT_DIR="/DATA/home/edu135/MNIST/$report_dir"
export HOLD_ECO_OUTPUT_DIR="/DATA/home/edu135/MNIST/$output_dir"
: "${HOLD_ECO_INPUT_BLOCK:=route_pg_ladder_vdd50_vss20_path507x55_h015}"
: "${HOLD_ECO_OUTPUT_BLOCK:=route_pg_ladder_hold_eco_open_site_m0}"
: "${HOLD_MARGIN:=0.00}"
: "${PHYSICAL_MODE:=open_site}"
: "${PT_EXEC:=/tools/synopsys/prime/W-2024.09-SP5-3/bin/pt_shell}"
export HOLD_ECO_INPUT_BLOCK
export HOLD_ECO_OUTPUT_BLOCK
export HOLD_MARGIN
export PHYSICAL_MODE
export PT_EXEC

icc2_shell -f 4_Backend_ICC2/0_Script/07_extract_sta/run_post_route_hold_eco_trial.tcl \
  | tee "$log_dir/run.log"
